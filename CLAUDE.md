...
## Private Fork Development Notes

This fork no longer keeps the Cursor Cloud bootstrap scripts or the S3-backed
`local-stack-binaries` cache workflow. Local frontend verification should use
`PORT=<free port> bun run dev` from `apps/web` against the dev backend. Backend
tests should still run from the repository root with `cargo test -p <crate>`.

Leave `SQLX_OFFLINE` unset for `cargo test`. If SQLx reports missing cached
query data, run `just prepare_db` instead of enabling offline mode.
