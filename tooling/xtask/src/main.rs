...
//!
//! Each xtask command lives in its own crate under `tooling/xtask/crates/` so
...
//! `cargo run --manifest-path .../tooling/xtask/Cargo.toml -- <command>` — while
...
/// (`<workspace>/tooling/xtask`) at compile time so the nested `cargo run`
